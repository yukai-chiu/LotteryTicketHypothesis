	...
	def __init__(self, kernel_size, dw):
		...
		if dw:
			self.convt1 = nn.Sequential(
				convt_dw(1024, kernel_size), pointwise(1024, 512)
			)
...
class ResNetSkipAdd(nn.Module):
	def __init__(self, layers, output_size, in_channels=3, pretrained_encoder=True):
...
class MobileNetSkipAdd(nn.Module):
	def __init__(self, output_size, pretrained_encoder=True):
		...
		self.decode_conv4 = nn.Sequential(
			depthwise(128, kernel_size), pointwise(128, 64)
		)
...
class MobileNetSkipConcat(nn.Module):
	def __init__(self, output_size, pretrained_encoder=True):
		...
		self.decode_conv4 = nn.Sequential(
			depthwise(256, kernel_size), pointwise(256, 64)
		)
		self.decode_conv5 = nn.Sequential(
			depthwise(128, kernel_size), pointwise(128, 32)
		)
		self.decode_conv6 = pointwise(32, 1)
		weights_init(self.decode_conv1)
		weights_init(self.decode_conv2)
